...
use agentmosaic_tools::{EditFile, ExecuteCommand, SearchDir, ViewFile, WriteFile};
use agentmosaic_workspace::{GitWorkspace, PostWriteGuard, ToolError, Workspace, MAX_VIEW_LINES};
...
fn temp_ws() -> (Workspace, PathBuf) {
    ...
    let dir = std::env::temp_dir().join(format!("agentmosaic_it_{seq}_{}", std::process::id()));
...
fn temp_git_repo() -> PathBuf {
    ...
    static SEQ: AtomicU32 = AtomicU32::new(0);
    let seq = SEQ.fetch_add(1, Ordering::Relaxed);
    let dir = std::env::temp_dir().join(format!("agentmosaic_it_git_{seq}_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let git = |args: &[&str]| {
