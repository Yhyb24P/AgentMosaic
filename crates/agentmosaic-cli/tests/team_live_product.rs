...
//! `am` binary — with a registered real Codex Lead
...
//!
//! ```text
//! cargo test -p agentmosaic-cli --test team_live_product -- --ignored --nocapture
//! ```
//!
...
fn unique_root(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "agentmosaic_team_live_{name}_{}_{}",
        std::process::id(),
        SystemTime::now()
...
/// Locate one of the runtime's binaries. `cargo test -p agentmosaic-cli` can
...
fn initialize_git_repo(repo: &Path) {
    ...
    let status = Command::new("git")
        .args([
            "-c",
            "user.email=agentmosaic@example.invalid",
            "-c",
