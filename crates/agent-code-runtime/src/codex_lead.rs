...
impl CodexLeadBrain {
    ...
    fn ensure_thread(&mut self) -> Result<String, LeadBrainError> {
        ...
        let started = server.initialize("ras-codex-lead", "0.1").and_then(|_| {
...
mod tests {
    ...
    use agent_code_team::{AgentMessage, AgentTaskResult, ArtifactMeta, LeadContext};
    ...
    fn delegate_reply_maps_to_specs() {
        ...
        let decision = brain
            ...
            .unwrap();
        match decision {
            agent_code_team::LeadDecision::Delegate(specs) => {
                assert_eq!(specs.len(), 2);
                assert_eq!(specs[0].objective, "scrape");
                assert_eq!(specs[0].kind, agent_code_team::TaskKind::Bulk);
                assert_eq!(specs[0].target.as_deref(), Some("worker-a"));
                assert!(specs[0].parent.is_none());
                assert!(specs[0].context.is_empty());
                assert_eq!(specs[1].kind, agent_code_team::TaskKind::Utility);
    ...
    fn complete_reply_maps_to_a_team_result() {
        ...
        match brain.parse_reply(&reply).unwrap() {
            agent_code_team::LeadDecision::Complete(result) => {
