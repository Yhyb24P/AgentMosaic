...
use agentmosaic_model::Observation;
use agentmosaic_tools::ToolRequest;
use agentmosaic_workspace::{CommandOutput, Workspace};
...
fn command_outcome(r: &agentmosaic_tools::ExecuteCommand, o: &CommandOutput) -> ToolOutcome {
...
fn command_output_text(r: &agentmosaic_tools::ExecuteCommand, o: &CommandOutput) -> String {
    let exit = o
        .exit_code
        .map(|c| c.to_string())
...
#[cfg(test)]
mod tests {
    use agentmosaic_tools::{ExecuteCommand, ToolRequest, ViewFile};
    ...
    fn ws_with(files: &[(&str, &str)]) -> (Workspace, std::path::PathBuf) {
        ...
        let dir =
            std::env::temp_dir().join(format!("agentmosaic_disp_{}_{seq}", std::process::id()));
