...
impl PersistedCodexTeamDriver {
    ...
    fn run_blocking(&self, task: AgentTask) -> Result<AgentTaskResult, String> {
        ...
        let bridge_log = std::env::temp_dir().join(format!(
            "agentmosaic_codex_team_{}_{}_{}.log",
            std::process::id(),
            task.id,
            attempt
        ));
        ...
        overrides.extend([
            ...
            format!(
                "mcp_servers.agentmosaic.env.AGENTMOSAIC_TASK_ID=\"{}\"",
                task.id
            ),
        ...
        let outcome = (|| {
            ...
            let thread = client
                .start_thread_with_developer_instructions(
                    ...
                    Some("For this bounded team task, use agentmosaic_request_context for teammate results. Tool arguments cannot select task or runtime identity."),
                )
                .map_err(|e| e.to_string())?;
            ...
            let turn = client
                .start_turn(
                    &thread,
                    &format!(
                        "Complete this bounded scheduler task: {}\n\nScheduler context:\n{}\n\nBefore relying on teammate results, invoke agentmosaic_request_context exactly once. Return a concise completion response after performing only the requested work.",
                        task.objective,
                        task.context.join("\n")
                    ),
                )
