...
codex_exec_runtime=PARTIAL (durable driver, JSONL normalization, bounded process group, thread binding, resume argv, deterministic scheduler test, managed strict worker-result schema with fail-closed parsing, and real read-only fresh plus same-thread `exec resume --json --output-schema` probes; full live scheduler-driver probe remains)
...
|---|---|---|---|---|---|---|---|---|---|
| Codex | `/home/yhshy/.local/bin/codex` | 0.154.0 | codex-exec | JSONL | PASS (logged-in local app-server interrupt, read-only `exec --json`, and fresh/resume real strict `--output-schema` probes, 2026-09-15) | PARTIAL (real strict worker results plus thread/usage; deterministic normalizer/driver coverage) | PASS (same foreign thread successfully resumed through real `exec resume --json --output-schema`) | PARTIAL (real app-server interrupt boundary passed) | exact exec contract pin; full driver/Lead live run remains pending |
