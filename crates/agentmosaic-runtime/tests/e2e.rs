...
use agentmosaic_core::{AgentState, Journal, Session, SessionId};
use agentmosaic_model::{ModelClient, ModelContext, ModelDecision, ModelError, Observation};
use agentmosaic_runtime::{AgentConfig, AgentError, AgentLoop};
use agentmosaic_storage::SqliteJournal;
use agentmosaic_tools::{EditFile, ExecuteCommand, ToolRequest, ViewFile};
...
fn temp_code_repo() -> std::path::PathBuf {
    ...
    let dir =
        std::env::temp_dir().join(format!("agentmosaic_e2e_repo_{}_{seq}", std::process::id()));
