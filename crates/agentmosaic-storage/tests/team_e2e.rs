...
async fn heterogeneous_team_end_to_end() {
    let path = std::env::temp_dir().join(format!("agentmosaic_team_e2e_{}.db", std::process::id()));
...
fn interrupted_task_can_be_recovered() {
    let path =
        std::env::temp_dir().join(format!("agentmosaic_team_crash_{}.db", std::process::id()));
    let _ = std::fs::remove_file(&path);
