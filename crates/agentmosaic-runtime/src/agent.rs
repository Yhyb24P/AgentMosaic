...
use agentmosaic_context::{
    build_compact_context, BytesTokenCounter, ContextBudget, HistorySink, HistorySource,
    TokenCounter,
};
use agentmosaic_core::{
    AgentState, Journal, JournalError, Session, SessionId, ToolCallId, ToolCallState,
};
use agentmosaic_model::{ModelClient, ModelContext, ModelDecision, ModelError, Observation};
use agentmosaic_tools::{ExecuteCommand, ToolRequest};
use agentmosaic_workspace::{GitWorkspace, ToolError, Workspace};
...
pub enum AgentError {
    ...
    Context(agentmosaic_context::ContextError),
...
impl From<agentmosaic_context::ContextError> for AgentError {
    fn from(e: agentmosaic_context::ContextError) -> Self {
        Self::Context(e)
    }
}
...
mod tests {
    ...
    use agentmosaic_context::{ContextBudget, ContextError, HistorySink, HistorySource};
    use agentmosaic_core::{InMemoryJournal, Session, SessionId};
    use agentmosaic_model::{ModelClient, ModelContext, ModelDecision, ModelError, Observation};
    use agentmosaic_tools::{ToolRequest, ViewFile};
    use agentmosaic_workspace::GitWorkspace;
    ...
    fn temp_git_repo() -> std::path::PathBuf {
        ...
        let dir =
            std::env::temp_dir().join(format!("agentmosaic_rt_git_{}_{seq}", std::process::id()));
    ...
    async fn verification_failure_loops_back_until_the_check_passes() {
        use agentmosaic_tools::EditFile;
    ...
    fn bounded_context_absorbs_json_escaping_growth() {
        use agentmosaic_context::{BytesTokenCounter, TokenCounter};
        use agentmosaic_model::{openai_request_body, OpenAiClient};
    ...
    fn bounded_context_fails_when_even_task_cannot_fit() {
        use agentmosaic_model::OpenAiClient;
        use std::time::Duration;
